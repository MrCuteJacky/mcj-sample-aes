public class AESAlgorithm {
    ...
    public byte[] encrypt(byte[] source) {
        ...
        int numRounds = 10 + (((secretKey.length() * 4 - 128) / 32));
        String line = toHex(padding(source, 16));
        int[][] state, initvector = new int[4][4];
        int[][] keymatrix = keySchedule(secretKey);
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                initvector[j][i] = Integer.parseInt(ivParameter.substring((8 * i) + (2 * j), (8 * i) + (2 * j + 2)), 16);
            }
        }
        state = new int[4][4];
        ...
        return toByteArray(matrixToString(state));
    }

    public byte[] decrypt(byte[] source) {

        int numRounds = 10 + (((secretKey.length() * 4 - 128) / 32));
        String line = toHex(source);
        ...
        return unPadding(toByteArray(matrixToString(state)));
    }
    ...
    public int[] scheduleCore(int[] in, int rconpointer) {
        leftRotate(in, 1);
        int hex;
    ...
    private int invMcHelper(int[][] arr, int i, int j) //Helper method for invMixColumns
    {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = AESAlgorithm.invgalois[i][k];
            int b = arr[k][j];
            mcsum ^= invMcCalc(a, b);
        }
        return mcsum;
    ...
    public byte[] padding(byte[] data, int blockSize) {
        ...
        for (int i = 0; i < padding; i++) {
            paddedData[data.length + i] = (byte) padding;
        }
        return paddedData;
    }
    ...
    public static byte[] toByteArray(String hexString) {
        ...
        for (int i = 0; i < byteArray.length; i++) {
            byteArray[i] = (byte) ((Character.digit(hexString.charAt(i * 2), 16) << 4) + Character.digit(hexString.charAt(i * 2 + 1), 16));
        }
